...
const getApi = (prefix) => {
  const raw = import.meta.env.VITE_API_URL || import.meta.env.VITE_TELEGRAM_CHANNELS_API_URL || import.meta.env.VITE_TELEGRAM_USERS_API_URL || "https://express-backend-myapp.onrender.com";
...
const extractCode = (v) => {
  ...
  try{
    // #/channel/join/CODE support
    if(t.includes("#")){
      const hash = t.split("#")[1] || "";
      const segs = hash.split("/").filter(Boolean);
      const j = segs.indexOf("join");
      if(j>=0 && segs[j+1]) return segs[j+1].split("?")[0].split("&")[0].split("#")[0];
      if(segs.length) return segs[segs.length-1].split("?")[0];
    }
    const u=new URL(t);
    const p=u.pathname.split("/").filter(Boolean);
    const j=p.indexOf("join");
    if(j>=0&&p[j+1]) return p[j+1].split("?")[0];
    return (p[p.length-1]||t).split("?")[0];
  }catch{
    const p=t.split("/").filter(Boolean);
    return (p[p.length-1]||t).split("?")[0].split("#")[0];
  }
};
...
const JoinChannelBox = ({ onChannelJoined, onOpenChannel }) => {
  ...
  const isValidInvite = (url) => {
    if (!url) return false;
    try { const u = new URL(url); return u.protocol.startsWith('http'); } catch { return url.length>=8; }
  };
  ...
  const handleJoin = async () => {
    ...
    if(!code || code.length < 6){ const m="Invalid share code"; setInlineErr(m); showCenter(m,"danger"); return; }

    setLoading(true);
    try {
      // ✅ retry for Render sleep
      const doFetch = async () => {
        return await fetch(`${ALLMISS_API}/join`, {
          method: "POST",
      ...
      try{ res = await doFetch(); }
        ...
        if(String(e.message).includes("Failed to fetch")){
          showCenter("Backend waking up... retrying","success");
      ...
      if (!res.ok) {
        throw new Error(data.message || (res.status===404 ? "Invalid link - code not found" : "Join failed"));
      }

      const channel = data.channel || data.data || data;
      
      setInviteUrl(""); setInlineErr("");
      showCenter(channel.channel_type==='private' ? "Private joined - PIN takun open kara" : "Channel joined successfully", "success");
      ...
      let m = msg;
      if(msg.toLowerCase().includes("failed to fetch")) m = "Backend not reachable - Render sleeping, 5 sec ne parat try kara";
      else if(msg.toLowerCase().includes("expire")) m = "Invitation link expired";
        ...
        <div className="jcc">
          ...
          <div className="jcr">
            <div className="jciw">
              ...
              <input ref={inputRef} value={inviteUrl} onChange={e=>{setInviteUrl(e.target.value); setInlineErr("");}} onKeyDown={e=>e.key==='Enter'&&handleJoin()} placeholder="Paste invitation link (https://react-myapp-omega.vercel.app/#/channel/join/...)" className="jci" />
            </div>
