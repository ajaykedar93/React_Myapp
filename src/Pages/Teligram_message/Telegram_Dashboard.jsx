// ✅ FINAL - Bottom Buttons Visible + Top Safe Small + Scrollable + Fast Reflect
...
const cacheChannelLogo = (channel) => {
  ...
  const transform = { logo_zoom: channel.logo_zoom, logo_x: channel.logo_x, logo_y: channel.logo_y };
...
const normalizeLogoUrl = (url="") => url.split('?')[0];
const hydrateChannels = (channels=[]) => {
  return (channels || []).map((channel) => {
    ...
    if(savedLogo) {
      ...
      if(!currentLogo || normalizeLogoUrl(currentLogo)===normalizeLogoUrl(savedLogo)) {
        hydrated = {...hydrated, logo_url: savedLogo, channel_logo_url: savedLogo, channel_logo: savedLogo };
      }
    }
    if(savedTransform) { try{ hydrated = {...hydrated,...JSON.parse(savedTransform)} }catch{} }
    return hydrated;
  });
};
...
function ProfileCard({ userData, onUpdateProfile, onSendOTP, onVerifyOTP }){
  ...
  const onPick=e=>{ const f=e.target.files?.[0]; if(!f) return; if(f.size>15*1024*1024) return showT('Max 15MB allowed','danger'); const r=new FileReader(); r.onload=()=>{ setAdjustSrc(r.result); setScale(1); setPos({x:0,y:0}); setShowAdjust(true); }; r.readAsDataURL(f); e.target.value=""; };
  ...
  const verifyOld=async()=>{ if(otpVal.old.length!==6) return showT('Enter 6 digit OTP','danger'); setLoading(true); try{ await onVerifyOTP?.(user.email,otpVal.old,'old'); setOtpStep('newEmail'); showT('Old email verified'); }catch(e){ showT(e.message||'Invalid OTP','danger'); } setLoading(false); };
  ...
  const handleSave=async(e)=>{
    e.preventDefault(); if(otpStep!=='none') return showT('Complete email change or cancel','danger');
    setLoading(true);
    ...
    setUser(optimisticUser); localStorage.setItem('telegram_user_details', JSON.stringify(optimisticUser)); setShowEdit(false); showT('Updating...');
    try{
      const payload={fullName:form.fullName,email:form.email,password:form.password||undefined,profileImage:form.file};
      const up=await onUpdateProfile?.(payload); const newImg=up?.profile_image_url||up?.profileImage||optimisticImg;
      ...
      setUser(fin); setImgVer(Date.now()); localStorage.setItem('telegram_user_details',JSON.stringify(fin)); showT('Profile updated');
    ...
    <>
      ...
      <Modal show={showEdit} onHide={()=>{ cancelEmailChange(); setShowEdit(false); }} centered dialogClassName="my-edit-modal" contentClassName="my-pop edit-safe-pop" backdrop="static">
        ...
        <Form onSubmit={handleSave} className="edit-safe-form">
          ...
          <Modal.Footer className="edit-safe-foot"><Button variant="light" size="sm" className="fbtn-cancel" onClick={()=>{ cancelEmailChange(); setShowEdit(false);}}>Cancel</Button><Button type="submit" size="sm" disabled={loading||otpStep!=='none'} className="my-pbtn fbtn-save">{loading?<Spinner size="sm"/>:'Save'}</Button></Modal.Footer>
        </Form>
      </Modal>
...
export default function Telegram_Dashboard(){
  ...
  const [user,setUser]=useState(()=>{ try{ const cached = JSON.parse(localStorage.getItem('telegram_user_details')||"null"); if(cached) return {telegram_user_id:cached.telegram_user_id,fullName:cached.full_name||cached.fullName,full_name:cached.full_name||cached.fullName,username:cached.username,email:cached.email,mobileNumber:cached.mobile_no,mobile_no:cached.mobile_no,profileImage:cached.profile_image_url,profile_image_url:cached.profile_image_url}; }catch{} return null; });
  const [loading,setLoading]=useState(!user); const [publicChannels,setPublicChannels]=useState([]); const [privateChannels,setPrivateChannels]=useState([]); const [toast,setToast]=useState({show:false,msg:'',type:'success'});
  ...
  const fetchAllData = useCallback(async (silent=false)=>{
    ...
    try{
      const r=await fetch(`${API_URL}/me`,{headers:{Authorization:`Bearer ${token}`}}); if(!r.ok) throw new Error();
      ...
      setUser(fmt); localStorage.setItem("telegram_user_details",JSON.stringify(u));
      const r2=await fetch(`${CHANNEL_API}/my-channels`,{headers:{Authorization:`Bearer ${token}`}}); if(r2.ok){ const d2=await r2.json(); const all = hydrateChannels(d2.channels||d2.data||[]); all.forEach(cacheChannelLogo); setPublicChannels(all.filter(c=>!(c.is_private||c.type==='private'||c.channel_type==='private'))||[]); setPrivateChannels(all.filter(c=>c.is_private||c.type==='private'||c.channel_type==='private')||[]); }
      if(!silent) setLoading(false);
    }catch(e){ if(!silent) setLoading(false); }
  },[]);
  useEffect(()=>{ const init=async()=>{ const token=getToken(); if(!token) return navigate("/telegram-login"); setLoading(true); await fetchAllData(true); setLoading(false); }; init(); },[navigate, fetchAllData]);
  useEffect(()=>{ const onVisible = ()=>{ if(document.visibilityState==='visible') fetchAllData(true); }; const onFocus = ()=> fetchAllData(true); window.addEventListener('focus', onFocus); document.addEventListener('visibilitychange', onVisible); return ()=>{ window.removeEventListener('focus', onFocus); document.removeEventListener('visibilitychange', onVisible); }; },[fetchAllData]);
  useEffect(()=>{ const tryAutoJoin=async()=>{ const token=getToken(); if(!token) return; const hashCode = window.location.hash.match(/\/channel\/join\/([^/?#]+)/)?.[1] || ""; const paramCode = params?.share_code || params?.code || ""; const pathCode = window.location.pathname.match(/\/channel\/join\/([^/]+)/)?.[1] || ""; const finalCode = (paramCode || hashCode || pathCode || "").trim(); if(!finalCode) return; try{ const r=await fetch(`${ALLMISS_API}/join`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`,"x-device-id":getDeviceId()},body:JSON.stringify({share_code:finalCode,code:finalCode,device_id:getDeviceId()})}); const d=await r.json(); if(!r.ok) throw new Error(d.message||"Invalid link"); showCenter(d.channel?.channel_type==="private"?"Private joined - PIN takun open kara":"Channel joined"); window.history.replaceState(null,"",window.location.pathname + window.location.search); fetchAllData(true); }catch(e){ showCenter(e.message,"danger"); } }; if(!loading) tryAutoJoin(); },[loading, params, fetchAllData]);
  ...
  const handleUpdateProfile=async(fd)=>{ const token=getToken(); const uid=user?.telegram_user_id; const f=new FormData(); if(fd.fullName) f.append("full_name",fd.fullName); if(fd.email&&fd.email!==user.email) f.append("email",fd.email); if(fd.password) f.append("password",fd.password); if(fd.profileImage) f.append("profile_image",fd.profileImage); const r=await fetch(`${API_URL}/${uid}`,{method:"PUT",headers:{Authorization:`Bearer ${token}`},body:f}); const res=await r.json(); if(!r.ok) throw new Error(res.message||"Update failed"); return res.user||res; };
  ...
  const handleOpenChannel = (ch) => { const cid = ch.channel_id || ch.id; if(!cid) return; localStorage.setItem('current_channel', JSON.stringify(ch)); localStorage.setItem('current_channel_id', String(cid)); navigate(`/channel/${cid}`); };
  ...
  return (
    <>
      ...
      <div className="dash-shell">
        <div className="dash-fixed-wrap"><div className="dash-fixed-inner"><ProfileCard userData={user} onUpdateProfile={handleUpdateProfile} onSendOTP={handleSendOTP} onVerifyOTP={handleVerifyOTP} /></div></div>
      ...
      <footer className="dev-footer"><span className="red-icon"><CodeSlash size={16} /></span><span>Developed By <b>Ajay Kedar</b></span></footer>
      ...
      .top-safe{position:fixed;top:0;left:0;right:0;height:var(--topSafe);background:#fff;z-index:1045}
       nav.navbar{position:fixed!important;top:var(--topSafe)!important;left:0!important;right:0!important;z-index:1044!important;height:var(--navH)!important;backdrop-filter:blur(18px)!important;background:rgba(255,255,255,0.96)!important;border-bottom:1px solid #e9eef5!important}
      ...
      .dash-scroll{flex:1;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:12px 12px 0}
      ...
      .bottom-safe-space{height:calc(48px + var(--botSafe));flex-shrink:0}
       ...
       /* ✅ PROFILE MODAL - TOP SMALL, BOTTOM SAFE VISIBLE */
      .my-edit-modal{margin:0!important;padding:0!important;display:flex!important;align-items:center!important;justify-content:center!important;min-height:100dvh!important}
      .my-edit-modal.modal-dialog{
         width:min(440px,94vw)!important;max-width:94vw!important;margin:auto!important;
         display:flex!important;align-items:center!important;justify-content:center!important;
         min-height:100dvh!important;
         padding:8px 12px calc(env(safe-area-inset-bottom) + 24px)!important; /* top small, bottom big */
       }
      .edit-safe-pop{
         width:100%!important;border:none!important;border-radius:20px!important;
         box-shadow:0 28px 80px rgba(15,23,42,.28)!important;
         display:flex!important;flex-direction:column!important;
         max-height:calc(100dvh - 16px - env(safe-area-inset-bottom) - env(safe-area-inset-top))!important;
         overflow:hidden!important;background:#fff!important;
         margin:auto!important;
       }
      .edit-safe-head{flex-shrink:0!important;padding:10px 16px!important;border-bottom:1px solid #f1f5f9!important;padding-top:calc(10px + env(safe-area-inset-top)*0.2)!important}
      .edit-safe-form{display:flex!important;flex-direction:column!important;flex:1!important;overflow:hidden!important;min-height:0!important}
      .edit-safe-body{
         flex:1!important;overflow-y:auto!important;overflow-x:hidden!important;
         -webkit-overflow-scrolling:touch!important;padding:16px!important;
         padding-bottom:12px!important;min-height:0!important;
       }
       /* ✅ FOOTER FIX - ALWAYS VISIBLE ABOVE NAVIGATION */
      .edit-safe-foot{
         flex-shrink:0!important;position:sticky!important;bottom:0!important;
         background:#fff!important;z-index:30!important;
         border-top:1px solid #f1f5f9!important;
         padding:12px 14px!important;
         padding-bottom:calc(14px + env(safe-area-inset-bottom) + 12px)!important; /* extra gap */
      ...
      .fbtn-cancel{height:40px!important;padding:0 18px!important;border-radius:12px!important;font-weight:700!important;background:#f8fafc!important;border:1px solid #e2e8f0!important}
      .fbtn-save{height:40px!important;padding:0 20px!important;border-radius:12px!important;font-weight:800!important}
      .safe-bottom-pad{height:8px;flex-shrink:0}
      ...
      .footer-bottom-safe{position:fixed;left:0;right:0;bottom:0;height:calc(var(--sab) + 10px);background:#ffffff;z-index:1059}
       ...
       @media(max-width:480px){
        .my-edit-modal{align-items:flex-end!important}
        .my-edit-modal.modal-dialog{
        ...
        .edit-safe-body{max-height:calc(92dvh - 88px - env(safe-area-inset-bottom))!important}
        .edit-safe-foot{
           padding:12px 16px!important;
           padding-bottom:calc(18px + env(safe-area-inset-bottom) + 16px)!important; /* never hide */
         }
