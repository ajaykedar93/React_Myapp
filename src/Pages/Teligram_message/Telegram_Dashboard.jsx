// ✅ FINAL - Fast UI Reflect + Behind API Load + Bottom Buttons Safe Visible
...
function ProfileCard({ userData, onUpdateProfile, onSendOTP, onVerifyOTP }){
  ...
  const handleSave=async(e)=>{
    ...
    try{
      ...
      setUser(fin); setImgVer(Date.now()); localStorage.setItem('telegram_user_details',JSON.stringify(fin));
      showT('Profile updated');
    }catch(er){ showT(er.message,'danger'); }
    setLoading(false);
  };
...
export default function Telegram_Dashboard(){
  ...
  // ✅ FAST REFLECT - UI first, API behind
  const fetchAllData = useCallback(async (silent=false)=>{
    ...
    try{
      ...
      if(r2.ok){
        ...
        const all = hydrateChannels(d2.channels||d2.data||[]);
    ...
    }catch(e){ console.log("silent sync failed", e.message); }
    finally{ if(!silent) setLoading(false); }
  },[]);

  useEffect(()=>{ const init=async()=>{ const token=getToken(); if(!token) return navigate("/telegram-login"); if(!user) setLoading(true); await fetchAllData(true); setLoading(false); }; init(); },[navigate, fetchAllData]);
  ...
  useEffect(()=>{
    const tryAutoJoin=async()=>{
      ...
      try{
        ...
        showCenter(d.channel?.channel_type==="private"?"Private joined":"Channel joined");
  ...
  const handleChannelCreated=(ch)=>{
    const isP=ch.is_private||ch.type==='private'||ch.channel_type==='private';
    const setChannels=isP?setPrivateChannels:setPublicChannels;
    // instant UI
    setChannels(p=>ch._replacePendingId?p.map(x=>String(x.channel_id||x.id)===String(ch._replacePendingId)?{...ch,_pending:false}:x):[ch,...p]);
    if(!ch._pending) setTimeout(()=>fetchAllData(true), 600);
  };
  ...
  return (
    <>
     ...
     .my-edit-modal.modal-dialog{width:min(440px,94vw)!important;max-width:94vw!important;margin:auto!important;display:flex!important;align-items:center!important;justify-content:center!important;min-height:100dvh!important;padding:8px 12px calc(env(safe-area-inset-bottom) + 24px)!important}
     .edit-safe-pop{width:100%!important;border:none!important;border-radius:20px!important;box-shadow:0 28px 80px rgba(15,23,42,.28)!important;display:flex!important;flex-direction:column!important;max-height:calc(100dvh - 16px - env(safe-area-inset-bottom) - env(safe-area-inset-top))!important;overflow:hidden!important;background:#fff!important;margin:auto!important}
     ...
     .edit-safe-body{flex:1!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;padding:16px!important;padding-bottom:12px!important;min-height:0!important}
     .edit-safe-foot{flex-shrink:0!important;position:sticky!important;bottom:0!important;background:#fff!important;z-index:30!important;border-top:1px solid #f1f5f9!important;padding:12px 14px!important;padding-bottom:calc(14px + env(safe-area-inset-bottom) + 12px)!important;display:flex!important;justify-content:flex-end!important;gap:10px!important;box-shadow:0 -6px 20px rgba(15,23,42,.06)!important}
     ...
     .footer-bottom-safe{position:fixed;left:0;right:0;bottom:0;height:calc(var(--sab) + 10px);background:#ffffff;z-index:1059}
       ...
       .my-edit-modal.modal-dialog{width:100%!important;max-width:100%!important;margin:0!important;margin-top:auto!important;padding:0!important;padding-top:calc(env(safe-area-inset-top) + 6px)!important;align-items:flex-end!important;min-height:100dvh!important}
       .edit-safe-pop{border-radius:20px 20px 0 0!important;max-height:calc(92dvh - env(safe-area-inset-bottom))!important;width:100%!important;margin-top:auto!important}
       .edit-safe-body{max-height:calc(92dvh - 88px - env(safe-area-inset-bottom))!important}
       .edit-safe-foot{padding:12px 16px!important;padding-bottom:calc(18px + env(safe-area-inset-bottom) + 16px)!important}
