...
export default function WorkDetails() {
  ...
  const startEdit = (id, name) => { setEditingId(id); setEditName(name); };
  ...
  const palette = {
    DPR: { bgActive: "linear-gradient(135deg,#DCFCE7,#bbf7d0)", fgActive: "#064e3b", border: "#86efac", shadow: "rgba(22,163,74,.30)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(134,239,172,.35), transparent)", underline: "#16a34a" },
    INWARD: { bgActive: "linear-gradient(135deg,#E0E7FF,#c7d2fe)", fgActive: "#1e3a8a", border: "#a5b4fc", shadow: "rgba(59,130,246,.28)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(147,197,253,.35), transparent)", underline: "#3b82f6" },
    "SITE KHARCH": { bgActive: "linear-gradient(135deg,#FEF9C3,#fde68a)", fgActive: "#713f12", border: "#fcd34d", shadow: "rgba(234,179,8,.32)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(251,191,36,.35), transparent)", underline: "#f59e0b" },
    CATEGORY: { bgActive: "linear-gradient(135deg,#FDE2E4,#fecdd3)", fgActive: "#7f1d1d", border: "#fca5a5", shadow: "rgba(239,68,68,.28)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(254,205,211,.42), transparent)", underline: "#ef4444" },
    "TOTAL KHARCH": { bgActive: "linear-gradient(135deg,#D1FAE5,#99f6e4)", fgActive: "#065f46", border: "#6ee7b7", shadow: "rgba(20,184,166,.30)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(110,231,183,.40), transparent)", underline: "#10b981" },
    "SiteKharch GET": { bgActive: "linear-gradient(135deg,#F1F5F9,#e2e8f0)", fgActive: "#0f172a", border: "#cbd5e1", shadow: "rgba(15,23,42,.22)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(148,163,184,.35), transparent)", underline: "#64748b" },
    "DPR GET": { bgActive: "linear-gradient(135deg,#FAE8FF,#f5d0fe)", fgActive: "#4a044e", border: "#f0abfc", shadow: "rgba(217,70,239,.30)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(240,171,252,.40), transparent)", underline: "#a21caf" },
    "INWARD GET": { bgActive: "linear-gradient(135deg,#E0F2FE,#bae6fd)", fgActive: "#0c4a6e", border: "#93c5fd", shadow: "rgba(14,165,233,.30)", glow: "radial-gradient(20px 12px at 50% 50%, rgba(186,230,253,.42), transparent)", underline: "#0284c7" },
  };
  ...
  const renderCategoryTab = () => (
    <div className="card shadow-sm p-3 mb-2">
      ...
      <form className="d-flex gap-2 mb-2 flex-wrap" onSubmit={addCategory}>
        <input
          type="text"
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case "DPR": return <AddDpr categories={categories} />;
      case "INWARD": return <AddInward categories={categories} />;
      case "SITE KHARCH": return <AddSitekharch categories={categories} />;
      case "SiteKharch GET": return <SitekharchGet categories={categories} />;
  ...
  return (
    <>
      ...
      <div className="wd-app">
        ...
        >
          <div className="container-fluid d-flex justify-content-between align-items-center px-0">
            ...
            <button
              ...
              style={{ color: "#1f2937", borderRadius: 10, boxShadow: "0 12px 26px rgba(2,6,23,.22)" }}
              onClick={() => {
        ...
        {/* Spacer so content starts beneath navbar */}
        <div className="wd-nav-spacer" />

        {/* Scroll area (tabs + content) */}
      ...
      <AnimatePresence>
        {confirm.open && (
          <motion.div
            ...
            className="modal show d-block" tabIndex="-1" role="dialog" aria-modal="true"
            style={{ background: "rgba(2,6,23,.55)" }}
          >
            <motion.div initial={{ y: 22, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 22, opacity: 0 }} transition={spring}
            ...
            >
              <div className="modal-content rounded-4 shadow-lg">
                ...
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={closeDelete} disabled={busy}>Cancel</button>
                  <button type="button" className="btn btn-danger" onClick={doDelete} disabled={busy}>Delete</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
