...
export default function CheckInwardView() {
  ...
  // ✅ Check mode + selected dates (NO blur logic)
      ...
      <div className="ivHeader">
        <div className="ivHeaderTop">
          ...
          <div className="ivActions">
            ...
            {/* ✅ Check Button (only shows checkbox UI) */}
            <button
              className={`ivCheckBtn ${checkMode ? "isOn" : ""}`}
              type="button"
              onClick={() => {
                setCheckMode((v) => !v);
                showToast(!checkMode ? "Check mode ON ✅" : "Check mode OFF ✅");
              }}
              title="Show date checkboxes"
            >
              Check
            </button>
        ...
        <>
          ...
          <div className="ivTableWrap" role="region" aria-label="Inward table scroll area">
            <table className="ivTable">
              ...
              <tbody>
                {groups.map((g) => {
                  const isThisDateChecked = selectedDates.has(g.date);
                      ...
                      {g.stores.map((storeGroup, sIdx) => (
                        <React.Fragment key={`${g.date}-${storeGroup.store}-${sIdx}`}>
                          {storeGroup.items.map((r, idx) => {
                            const showDateRow = sIdx === 0 && idx === 0; // ✅ only first row shows checkbox + date
                            const showStore = idx === 0;
                            const letter = String.fromCharCode(97 + idx);

                            return (
                              <tr
                                key={`${g.date}-${storeGroup.store}-${idx}-${r.inward_id}`}
                                className={isThisDateChecked ? "ivCheckedRow" : ""}
                              >
                                <td className="ivSr">{showDateRow ? g.srNo : ""}</td>

                                <td className="ivDate">
                                  {showDateRow ? (
                                    <div className="ivDateCell">
                                      {checkMode && (
                                        <input
                                          type="checkbox"
                                          className="ivDateCheck"
                                          checked={isThisDateChecked}
                                          onChange={() => toggleDate(g.date)}
                                          aria-label={`Check date ${formatDDMMYYYY(g.date)}`}
                                        />
                                      )}
                                      <span className="ivDateText">{formatDDMMYYYY(g.date)}</span>
                                    </div>
                                  ) : (
                                    ""
                                  )}
...
/* Body should not fight with wrapper scroll */
...
.ivDateText{ font-weight:900; }

/* ✅ ONLY checked date rows have strike */
.ivCheckedRow td{
  text-decoration: line-through;
  text-decoration-thickness: 2px;
  text-decoration-color: rgba(17,24,39,.6);
}
